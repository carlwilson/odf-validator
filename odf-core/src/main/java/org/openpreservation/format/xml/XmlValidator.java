...
public class XmlValidator {
    ...
    private final OdfSchemaFactory schemaFactory = new OdfSchemaFactory();
    ...
    public ValidationResult validate(final ParseResult parseResult, final InputStream toValidate,
            final Namespaces namespace, final Version version)
            throws IOException {
        if (!parseResult.isWellFormed()) {
            return ValidationResultImpl.of(parseResult, false, new ArrayList<>());
        }
        final List<Message> messages = new ArrayList<>();
        Schema schema = schemaFactory.getSchema(namespace, version);
